...
BACKGROUND_STD_THRESHOLD = 0.015         # Standard deviation of the background must be smaller than 1.5 cm/yr
...
SKIP_IDS = {'ls_45','ls_82','ls_1155','ls_1163','ls_1170','ls_1312','ls_131','ls_1322','ls_1329',
            'ls_163','ls_190','ls_225','ls_242','ls_250','ls_283','ls_303','ls_314','ls_357',
            'ls_395','ls_407','ls_419','ls_485','ls_489','ls_525','ls_984',}
...
def process_landslide(ls_row, vel_lons, vel_lats, lon_1d, lat_1d,
    ...
    # Post EQ 9 months 
    ls_row['ts_eq1_9month_vel_myr'], ls_row['ts_eq1_9month_err_myr'] = compute_velocity(ts_df["dates"].values, ts_df["clean_ts"].values, start=eq1, stop=eq1+9/12)
    ls_row['ts_eq2_9month_vel_myr'], ls_row['ts_eq2_9month_err_myr'] = compute_velocity(ts_df["dates"].values, ts_df["clean_ts"].values, start=eq2, stop=eq2+9/12)
    ...
    # Post EQ 3-6 months 
    ls_row['ts_eq1_3-6month_vel_myr'], ls_row['ts_eq1_3-6month_err_myr'] = compute_velocity(ts_df["dates"].values, ts_df["clean_ts"].values, start=eq1+3/12, stop=eq1+6/12)
    ls_row['ts_eq2_3-6month_vel_myr'], ls_row['ts_eq2_3-6month_err_myr'] = compute_velocity(ts_df["dates"].values, ts_df["clean_ts"].values, start=eq2+3/12, stop=eq2+6/12)
    
    # Post EQ 6-9 months 
    ls_row['ts_eq1_6-9month_vel_myr'], ls_row['ts_eq1_6-9month_err_myr'] = compute_velocity(ts_df["dates"].values, ts_df["clean_ts"].values, start=eq1+6/12, stop=eq1+9/12)
    ls_row['ts_eq2_6-9month_vel_myr'], ls_row['ts_eq2_6-9month_err_myr'] = compute_velocity(ts_df["dates"].values, ts_df["clean_ts"].values, start=eq2+6/12, stop=eq2+9/12)
    
    # Post EQ 9-12 months 
    ls_row['ts_eq1_9-12month_vel_myr'], ls_row['ts_eq1_9-12month_err_myr'] = compute_velocity(ts_df["dates"].values, ts_df["clean_ts"].values, start=eq1+9/12, stop=eq1+12/12)
    ls_row['ts_eq2_9-12month_vel_myr'], ls_row['ts_eq2_9-12month_err_myr'] = compute_velocity(ts_df["dates"].values, ts_df["clean_ts"].values, start=eq2+9/12, stop=eq2+12/12)

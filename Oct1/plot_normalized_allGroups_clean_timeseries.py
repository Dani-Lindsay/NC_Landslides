...
def summarize_panel(df_paths, method="minmax", window=5):
    ...
    for _, row in df_paths.iterrows():
        try:
            dates, ts, sign = load_timeseries(row["file_y"])
            disp_series.append(
...
def plot_group_comparisons(df, groupings, eq_dates, group_orders=None):
    ...
    for ax, (name, func) in zip(axes, groupings.items()):
        ...
        for color, g in zip(colors, group_list):
            sub = merged[(merged["group"] == g) & merged["file_y"].notna()][["ls_id", "file_y"]].copy()
...
id_candidates = [c for c in df.columns if re.search(r"ls_id", c)]
if not id_candidates:
    raise RuntimeError("Could not find a landslide ID column")
csv_id_col = id_candidates[0]
...
fig.suptitle(f" {label} Smoothed median comparison across grouping strategies", fontsize=12, y=0.985)
plt.tight_layout(rect=[0, 0, 1, 0.97])

out_png = os.path.join(fig_dir, f"norm_ts_all_medians_comparison_{label}_multiple{vel_multiple}_velmin_{vel_min_threshold}.png")
out_pdf = out_png.replace(".png", ".pdf")
fig.savefig(out_png, dpi=300, bbox_inches="tight")
fig.savefig(out_pdf, bbox_inches="tight")
plt.close(fig)

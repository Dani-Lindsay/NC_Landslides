...
vel_multiple      = 4        # velocity-ratio bounds
NORM_METHOD       = "minmax"
...
BOOT_N                 = 10            # bootstrap draws (set to 1000+ for final numbers)
...
def subset_muchFaster_muchHigher():
    ...
    df["vel_dry1"] = np.abs(df[f"ts_eq1_{label}_vel_myr"] * 100)
    df["vel_dry2"] = np.abs(df[f"ts_eq2_{label}_vel_myr"] * 100)
    ...
    for fp in h5_files:
        try:
            with h5py.File(fp, "r") as hf:
                ...
                recs.append({"ls_id": sid, "file_y": fp})
        except Exception:
            pass
...
def main():
    ...
    out_png=os.path.join(FIG_DIR,f"subset_muchFaster_muchHigher_withscatter_prequakewindow{PRE_WINDOW_YEARS}_nBoots{BOOT_N}_{label}.png")
    ...
    out_csv = os.path.join(
        FIG_DIR,
        f"subset_muchFaster_muchHigher_fit_summary_prequakewindow{PRE_WINDOW_YEARS}_nBoots{BOOT_N}_{label}.csv"
    )

...
# -*- coding: utf-8 -*-
"""
Velocity vs Precipitation grouping analysis (Option 2 thresholds).
Groups landslides by precipitation ratio, plots velocities colored by precipitation bins.
"""
...
# -------------------------
def categorize_precip_group(ratio):
    if ratio <= 1.1:
        return "Similar"
    elif 1.1 < ratio <= 1.5:
        return "Higher"
...
df["group"] = df["precip_ratio"].apply(categorize_precip_group)

# -------------------------
# Colors: use diverging palette reversed (Higher = red, Lower = blue)
# -------------------------
group_order = ["Similar", "Higher", "Much Higher"]
colors = sns.color_palette("Spectral", n_colors=len(group_order))[::-1]
roma_colors = dict(zip(group_order, colors))

# =========================
# Scatter plot (vel_dry1 vs vel_dry2, colored by precipitation group)
# =========================
...
ax.set_title("Velocity Comparison by Precipitation Ratio Group", fontsize=10)
...
ax.legend(
    handles=[plt.Line2D([0], [0], marker='o', color='w',
             ...
             for g in group_order],
    title="Precip Ratio Group", fontsize=9, title_fontsize=10,
    loc="upper right"
)

plt.tight_layout()
plt.savefig(f"{fig_dir}/Fig_Velocity_scatter_byPrecip_opt2.png", dpi=300)
plt.savefig(f"{fig_dir}/Fig_Velocity_scatter_byPrecip_opt2.jpeg", dpi=300)
plt.savefig(f"{fig_dir}/Fig_Velocity_scatter_byPrecip_opt2.pdf")
plt.show()
...
# =========================
# Violin plot (velocity ratio by precipitation group, clipped)
# =========================
...
ax.set_xlabel("Precip Ratio Group", fontsize=10)
ax.set_ylabel("Velocity Ratio (WY23/WY22)", fontsize=10)
ax.set_title("Velocity Ratio Distribution by Precipitation Group (clipped at 99th percentile)", fontsize=10)
ax.tick_params(axis="x", rotation=0)

plt.tight_layout()
fig.savefig(f"{fig_dir}/Fig_VelocityRatio_violin_byPrecip_opt2_clipped.png", dpi=300)
fig.savefig(f"{fig_dir}/Fig_VelocityRatio_violin_byPrecip_opt2_clipped.jpeg", dpi=300)
fig.savefig(f"{fig_dir}/Fig_VelocityRatio_violin_byPrecip_opt2_clipped.pdf")
plt.show()

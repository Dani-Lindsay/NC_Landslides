...
def cross_correlation(dates, disp, rain, start, end, max_lag_days=120, sm_window=5):
...
def analyze_group(group_name, subdf):
    ...
    for _,row in subdf.iterrows():
        try:
            ...
            disp_s = pd.Series(ts, index=pd.to_datetime(dates))
    ...
    for (period,label,ax_cc,ax_ts) in [
        ...
        disp_win = normalize_minmax(disp_med[mask].values)
        rain_win = normalize_minmax(rain_med[mask].values)
        dates_win = disp_med.index[mask]

        lags,corr,best_lag,best_corr,_ = cross_correlation(
            dates_win, disp_win, rain_win,
        ...
        ax_ts.plot(dates_win, disp_win, "b-", lw=2, label="Median disp")
        q05 = disp_df[mask].quantile(0.05, axis=1)
        q95 = disp_df[mask].quantile(0.95, axis=1)
        ax_ts.fill_between(dates_win, normalize_minmax(q05.values),
                           normalize_minmax(q95.values), alpha=0.3,
                           color="blue", label="5–95%")
        ax_ts.plot(dates_win, rain_win, "g-", alpha=0.6,
                   label="Cumulative rain (norm.)")
